...
export default function HomePageClient({ heroBook, newArrivals }: HomePageClientProps) {
    ...
    async function fetchMiniBookshelf() {
      try {
        ...
        const visibleBooks = myBooks.filter((book) => {
          ...
          const isFree =
            downloadUrl.includes('orthodoxbookshop') || epubUrl.includes('orthodoxbookshop');
  ...
  return (
    <div>
                  ...
                  <div className="flex items-start gap-4 p-6 rounded-xl bg-parchment border border-parchment-dark/30 hover:border-gold/50 hover:shadow-md transition-all">
                    ...
                    <div className="flex-grow min-w-0">
                      ...
                      <p className="mt-1 text-sm text-ink-light leading-relaxed">
                        {t(
                          CATEGORY_DESC_KEYS[category.slug] || 'categories.defaultDesc',
                          { name: tGlobal(category.nameKey) }
                        )}
                      </p>
      ...
      <section className="pt-8 md:pt-10 pb-16 md:pb-20 bg-parchment" aria-labelledby="homepage-features">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <h2 id="homepage-features" className="sr-only">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            <div className="text-center">
              ...
              <p className="text-ink-light">{t('features.curatedDesc')}</p>
            </div>
            <div className="text-center">
              <div className="w-16 h-16 bg-burgundy/10 rounded-full flex items-center justify-center mx-auto mb-4">
                <svg className="w-8 h-8 text-burgundy" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <p className="text-ink-light">{t('features.qualityDesc')}</p>
            </div>
            <div className="text-center">
              <div className="w-16 h-16 bg-burgundy/10 rounded-full flex items-center justify-center mx-auto mb-4">
                <svg className="w-8 h-8 text-burgundy" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <p className="text-ink-light">{t('features.churchDesc')}</p>
            </div>
          </div>
        </div>
      </section>
