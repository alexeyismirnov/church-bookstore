            ...
            {book && (
              <>
                <div className="relative z-10 h-full flex items-center justify-center">
                  <Link href={`/product/${book.id}`} className="h-full">
                    <img
                      src={book.coverImage}
                      alt={book.title}
                      className="max-h-[96%] max-w-full w-auto rounded-2xl shadow-2xl transform rotate-3 hover:rotate-0 transition-transform duration-500"
                    />
                  </Link>
                </div>
                {/* Decorative Elements */}
