...
import { buildCatalogRequestKey } from '../../lib/metadata';
import Breadcrumbs from '../../components/Breadcrumbs';
import type { Book } from '../../types';
import type { CatalogContentSeoProps, CatalogInitialData } from './types';

interface CatalogContentProps extends CatalogContentSeoProps {
  categoryId?: string;
...
export default function CatalogContent({
  ...
  // Fetch products when filters change; skip when SSR data still matches URL + locale/currency
  useEffect(() => {
    if (isCurrencyLoading) {
      return;
    }

    const currentKey = buildCatalogRequestKey({
      category: selectedCategoryId ?? categoryParam ?? undefined,
      q: isSearchActive ? committedSearchQuery : undefined,
      page: currentPage,
      inStock,
      locale,
      currency,
  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 pb-16">
        ...
        <div className="mb-8">
          <h2 className="text-2xl md:text-3xl font-bold text-ink font-display">{listingTitle}</h2>
          {categoryIntro && (
            <p className="mt-3 text-ink-light leading-relaxed max-w-3xl">{categoryIntro}</p>
          )}
        </div>
