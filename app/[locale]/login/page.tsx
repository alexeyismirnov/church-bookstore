...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (success) {
      // After login, cacheProfilePreferences() in AuthContext has already written
      // the user's profile language to localStorage. Read it from there instead of
      // using the stale React locale state from the current URL.
      const cachedLocale = localStorage.getItem('locale');
      const effectiveLocale: Locale =
        cachedLocale && locales.includes(cachedLocale as Locale)
          ? (cachedLocale as Locale)
          : (params.locale as Locale);

      // Redirect to the URL specified in the `redirect` query param, or home
      const redirectTo = searchParams.get('redirect') || '/';
      const target = redirectTo.startsWith('/')
        ? `/${effectiveLocale}${redirectTo}`
        : redirectTo;
      router.push(target);
      router.refresh();
    } else {
    ...
    <div className="min-h-screen bg-background py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-md w-full mx-auto">
        ...
        <p className="text-center text-gray-600 mt-6">
          {t('noAccount')}{' '}
          <LocalizedLink href="/register" className="text-burgundy hover:underline font-medium">
            {t('register')}
          </LocalizedLink>
        </p>
      </div>
